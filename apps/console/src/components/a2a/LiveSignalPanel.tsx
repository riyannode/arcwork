    ...
    <div className="rounded border border-white/5 bg-white/[0.02] p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-[9px] uppercase tracking-widest text-[#555]">{row.window}</span>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="rounded border border-cyan-500/15 bg-cyan-950/[0.06] p-2.5">
          <p className="font-mono text-[8px] uppercase tracking-widest text-cyan-300/70 mb-1.5">Ignia · raw signal</p>
          ...
          <div className="mt-1.5 space-y-0.5">
            {row.ignia.features.slice(0, 3).map((f, i) => (
              <p key={i} className="font-mono text-[8px] text-[#555] truncate">• {f}</p>
            ))}
          </div>
        </div>
        ...
        <div className="rounded border border-violet-500/15 bg-violet-950/[0.06] p-2.5">
          <p className="font-mono text-[8px] uppercase tracking-widest text-violet-300/70 mb-1.5">Apolo · decision</p>
          ...
          <div className="flex items-center gap-1.5 mt-1">
            ...
            <span className={`font-mono text-[8px] ${RISK_COLORS[row.apolo.risk] ?? 'text-[#555]'}`}>
              risk: {row.apolo.risk}
            </span>
          </div>
          <p className="mt-1.5 font-mono text-[8px] text-[#555] leading-[1.4]">{row.apolo.reason}</p>
        </div>

        {/* Hermes */}
        <div className="rounded border border-amber-500/15 bg-amber-950/[0.06] p-2.5">
          <p className="font-mono text-[8px] uppercase tracking-widest text-amber-300/70 mb-1.5">Hermes · action</p>
          <p className={`font-mono text-lg font-semibold ${row.hermes.action === 'SKIP' ? 'text-zinc-400' : row.hermes.action === 'BUY_UP' ? 'text-emerald-400' : 'text-rose-400'}`}>
          ...
          <div className="mt-1.5 space-y-1">
            ...
            <div className="flex justify-between font-mono text-[9px]">
              ...
              <span className="text-cyan-300/80">{row.hermes.mode}</span>
            </div>
            <div className="flex justify-between font-mono text-[9px]">
              <span className="text-[#555]">x402 paid</span>
              <span className="text-emerald-300/80">0.01 USDC</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Market context */}
      <div className="flex items-center gap-3 font-mono text-[9px] text-[#555]">
        ...
        <a
          ...
          className="ml-auto text-[#7A7A7A] hover:text-[#C5A67C]"
        >
          polymarket ↗
        </a>
      </div>
    ...
    <div className="rounded border border-white/5 bg-white/[0.015] p-5">
      ...
      {rows.length === 0 && !error ? (
        <div className="rounded border border-dashed border-white/10 p-6 text-center">
          <p className="font-mono text-[10px] text-[#555]">loading signal pipeline…</p>
        </div>
      ) : (
