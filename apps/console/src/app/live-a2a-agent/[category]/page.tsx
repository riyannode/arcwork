  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#050505] px-4 py-5 text-[#EAE4D8] selection:bg-[#C5A67C]/20 sm:px-6 lg:px-8">
      ...
      <div className="relative mx-auto flex max-w-[1480px] flex-col gap-4">
        <header className="overflow-hidden rounded-sm border border-[#C5A67C]/15 bg-[#0A0A0A]/90">
          <div className="flex flex-col gap-3 border-b border-white/10 px-5 py-4 lg:flex-row lg:items-center">
            ...
            <div className="ml-auto flex flex-wrap items-center gap-2">
              ...
              <Chip tone="cyan">{category.feeRange}</Chip>
        ...
        <TerminalPanel title="Category Spec" right={<Chip tone={category.status === 'LIVE' ? 'green' : 'amber'}>{category.status}</Chip>}>
          <div className="grid gap-px bg-[#C5A67C]/10 lg:grid-cols-[1fr_.75fr]">
            <div className="bg-[#0A0A0A]/90 p-4">
              <div className="mb-3 font-mono text-[10px] uppercase tracking-[0.2em] text-[#EAE4D8]/60">Capabilities</div>
              <div className="grid gap-2 sm:grid-cols-2">
                {category.capabilities.map((cap) => (
                  <div key={cap} className="rounded-sm border border-white/10 bg-black/25 p-3 text-sm text-[#EAE4D8]/75">{cap}</div>
                ))}
              </div>
            </div>
            <div className="bg-[#0A0A0A]/90 p-4">
              <div className="mb-3 font-mono text-[10px] uppercase tracking-[0.2em] text-[#EAE4D8]/60">Agent examples</div>
              <div className="space-y-2">
                {category.exampleAgents.map((agent) => (
                  <div key={agent} className="rounded-sm border border-white/10 bg-black/25 p-3 font-mono text-xs text-[#EAE4D8]">{agent}</div>
                ))}
              </div>
              <Link href={`/register?category=${category.key}`} className="mt-3 inline-flex rounded-sm border border-[#C5A67C]/35 bg-[#C5A67C]/10 px-3 py-2 font-mono text-[10px] uppercase tracking-[0.2em] text-[#C5A67C] transition hover:bg-[#C5A67C]/15">
                Register agent →
              </Link>
            </div>
          </div>
        ...
        {isPredictionMarket ? (
          <>
            <TerminalPanel
              ...
              right={<Chip tone={wsState === 'live' ? 'green' : 'amber'}>{wsState === 'live' ? 'RTDS LIVE' : wsState.toUpperCase()}</Chip>}
              className="min-h-[480px]"
            >
              <LiveMarketWidget book={activeBook} stream={activeStream} asset={orderbookAsset} marketVolume={activeMarket?.volume} />
            </TerminalPanel>

            <TerminalPanel title="Signal Stream" right={<Chip tone="cyan">3 ROWS · SHOW ALL</Chip>} className="min-h-[180px]">
