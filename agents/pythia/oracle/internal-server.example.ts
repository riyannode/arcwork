...
const PORT = Number(process.env.PYTHIA_ORACLE_INTERNAL_PORT ?? 4011);
const INTERNAL_KEY = process.env.PYTHIA_ORACLE_INTERNAL_KEY;
...
app.get('/health', (_req, res) => res.json({ ok: true, agent: 'PythiaOracle', publicPaid: false }));

app.listen(PORT, () => console.log(`[PythiaOracle] internal raw-signal server on :${PORT}`));
